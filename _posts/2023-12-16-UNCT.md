...
- 마르코프 모델: 미래의 상태$S_{t+1}$은 현재 상태 $S_t$에 영향을 받고, 과거 상태 ($S_{t-1}, S_{t-2},...$) 에는 영향을 받지 않는 시스템에 대한 확률모델<br>

따라서 $P(S_{t+1}|S_t, S_{t-1},...,S_0) = P(S_{t+1}|S_t)$
<br>
- Hidden의 의미는 마르코프 모델의 state(우리가 알고싶어하는 것)가 숨어있다는 것을 뜻한다.<br>
- 관측치는 볼 수 있는데, 우리가 알고 싶은 state와는 다르다.<br>
<br>

HMM은 3가지의 파라미터로 구성되어 있다.<br>
$\lambda = (A , B , \pi)$<br>
